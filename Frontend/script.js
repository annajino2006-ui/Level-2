...
function renderKeyboard() {
    ...
    const buttons =
        keyboardElement.querySelectorAll(".key");
...
function updateKeyboardStates(guess, result) {
    ...
    guess.split("").forEach((letter, index) => {
        ...
        if (
            !oldState ||
            priority[newState] > priority[oldState]
        ) {
...
async function submitGuess() {
    ...
    try {
        ...
        if (data.status === "win") {
            ...
            showStatus(
                `You solved it! Score: ${data.score}`,
                "success"
            );
        ...
        if (data.status === "lose") {
            ...
            showStatus(
                `Out of guesses! The word was ${data.secretWord}.`,
                "error"
            );
        ...
        showStatus(
            `Guess ${data.attempts} of ${data.maxAttempts}`
        );
...
async function resetGame() {
    ...
    try {
        ...
        showStatus(
            "Use the clues to find the hidden word. You have 6 tries."
        );
